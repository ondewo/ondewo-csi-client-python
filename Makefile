...
generate_ondewo_protos:
	...
	for f in $$(find ${ONDEWO_SIP_API_DIR}/ondewo/sip -name '*.proto'); do \
		python -m grpc_tools.protoc -I ${ONDEWO_SIP_API_DIR} --python_out=${PROTO_OUTPUT_FOLDER} --mypy_out=${PROTO_OUTPUT_FOLDER} --grpc_python_out=${PROTO_OUTPUT_FOLDER} $$f; \
	done
