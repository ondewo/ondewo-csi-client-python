...
This repo has submodules (`ondewo-csi-api`, `ondewo-proto-compiler`). If a submodule's working
...
```text
Release ONDEWO CSI Python Client ${ONDEWO_CSI_VERSION}
```

So the heading of a new entry must read exactly `## Release ONDEWO CSI Python Client <version>`. **This wording is
...
```bash
grep -c '^## Release ONDEWO CSI Python Client ' RELEASE.md     # must be >= 1 for your new version
```
...
This package publishes as **`ondewo-csi-client`**, which is not always the repository name — the JS client
...
```bash
curl -s https://pypi.org/pypi/ondewo-csi-client/json | python3 -c "import sys,json;print(json.load(sys.stdin)['info']['version'])"
```
